import sys, time, random, threading, ctypes
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.ivl = QtWidgets.QLineEdit("10"); form.addRow("Interval (sec):",   self.ivl)
        self.mic = QtWidgets.QLineEdit("1");  form.addRow("Mickeys (pixels):", self.mic)

        self.status = QtWidgets.QLabel("Idle"); form.addRow("Status:", self.status)

        h = QtWidgets.QHBoxLayout()
        self.btn_start = QtWidgets.QPushButton("Start");  h.addWidget(self.btn_start)
        self.btn_stop  = QtWidgets.QPushButton("Stop");   h.addWidget(self.btn_stop)
        form.addRow(h)
    ...
    def start(self):
        try:
            ...
            m = int(self.mic.text())
            # Enforce minimum 5 seconds interval
            if i < 5.0:
                i = 5.0
                self.ivl.setText("5")
            if d <= 0 or m == 0:
